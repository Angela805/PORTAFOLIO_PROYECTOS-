...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($categoria) || $cantidad === 0 || $precio === 0.0 || $proveedor_id === 0) {
        ...
        if ($stmt->execute()) {
            ...
            // Limpiar variables para resetear el formulario al tener éxito
            $nombre = $categoria = $precio = $proveedor_id = '';
            $cantidad = 1; 

        } else {
            // **CORRECCIÓN DE SEGURIDAD CRÍTICA**
            // NO mostrar $stmt->error. Mostrar un mensaje genérico al usuario.
            // El error real debería ser logged internamente.
